...
const INITIAL_ROADMAP = [
  ...
  {
    id: 2,
    title: 'Месяц 2: Базовое программирование',
    tasks: [
  ...
  {
    id: 3,
    title: 'Месяц 3: Работа с окружением и данными',
    tasks: [
  ...
  {
    id: 4,
    title: 'Месяц 4: Продвинутые концепции',
    tasks: [
  ...
  {
    id: 5,
    title: 'Месяц 5: Знакомство с Фреймворками',
    tasks: [
  ...
  {
    id: 6,
    title: 'Месяц 6: Базы данных и Авторизация',
    tasks: [
  ...
  {
    id: 9,
    title: 'Месяц 9: Инструменты CI/CD и Линтеры',
    tasks: [
  ...
  {
    id: 10,
    title: 'Месяц 10: Архитектура и Рефакторинг',
    tasks: [
  ...
  {
    id: 11,
    title: 'Месяц 11: Деплой и Портфолио',
    tasks: [
  ...
  {
    id: 12,
    title: 'Месяц 12: Поиск работы и собеседования',
    tasks: [
...
const INITIAL_TESTS = [
  { id: 1, name: 'Общий тест на IT-профессию', date: '20.05.2026', result: 'Frontend Developer (85%)' },
  { id: 2, name: 'Общий тест на IT-профессию', date: '12.05.2026', result: 'QA Engineer (70%)' }
];
...
function Dashboard({ user, onLogout }) {
  ...
  // Инициализация активной вкладки по query-параметру tab
  const queryParams = new URLSearchParams(location.search);
  const [activeTab, setActiveTab] = useState(queryParams.get('tab') || 'progress');

  // Синхронизация вкладки при переходе с параметром
  useEffect(() => {
  ...
  const handleCheckboxChange = (monthId, taskIndex) => {
    const key = `${monthId}-${taskIndex}`;
  ...
  const totalTasksCount = INITIAL_ROADMAP.reduce((acc, month) => acc + month.tasks.length, 0);
  const checkedTasksCount = Object.values(checkedTasks).filter(Boolean).length;
  const progressPercent = Math.round((checkedTasksCount / totalTasksCount) * 100);
    ...
    <section className="view active">
      <div className="dashboard-grid">
        ...
        <aside className="dashboard-sidebar">
          ...
          <button 
            className={`dashboard-tab-btn ${activeTab === 'tests' ? 'active' : ''}`}
            ...
            <div>
              ...
              <div className="profile-summary-card">
                ...
                <div className="profile-info">
                  <h3>{user?.name || 'Иван Иванов'}</h3>
                  <span className="profile-badge">Frontend Developer</span>
                </div>
              </div>
              ...
              <div className="stats-grid">
                ...
                <div className="stat-card card-blue">
                  ...
                  <div className="stat-value">{checkedTasksCount} / {totalTasksCount}</div>
                </div>
                      ...
                      {isOpen && (
                        <div className="roadmap-month-content">
                          {month.tasks.map((task, idx) => {
                            const isChecked = !!checkedTasks[`${month.id}-${idx}`];
                            return (
                              <label 
                                key={idx} 
                                className={`roadmap-checklist-item ${isChecked ? 'checked' : ''}`}
                              >
                                <input 
                                  type="checkbox" 
                                  checked={isChecked}
                                  onChange={() => handleCheckboxChange(month.id, idx)}
                                />
                                <span>{task}</span>
                              </label>
                            );
                ...
                {INITIAL_TESTS.map((test) => (
                  <div key={test.id} className="history-item">
                    <div>
                      ...
                      <div className="item-desc">Дата прохождения: {test.date}</div>
                    </div>
